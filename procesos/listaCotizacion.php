...
?>
			...
			<!-- <div id="barraCAM"> -->
				<?php
					...
					while($rowUsr=mysqli_fetch_array($bdUsr)){
							if($rowUsr['nPerfil'] != 'WM' and intval($rowUsr['nPerfil']) != 0 and $rowUsr['nPerfil'] != 3 and $rowUsr['nPerfil'] != 4){
								echo '<a class="btn btn-primary" href="plataformaCotizaciones.php?usrFiltro='.$rowUsr['usr'].'" title="'.$rowUsr['usuario'].'">'.$rowUsr['usr'].'</a>';
							}
					}
					$link->close();
					echo '<a class="btn btn-primary" href="plataformaCotizaciones.php?usrFiltro=Baja" 		title="Dadas de Baja">De Baja</a>';
					echo '<a class="btn btn-primary" href="plataformaCotizaciones.php?usrFiltro=" 			title="Todos">Todos</a>';
					echo '<a class="btn btn-primary" href="historial.php?usrFiltro=Historial" 				title="Historial">Historial</a>';
				?>
				</div>
				...
				<select class="form-control" name="empFiltro" onChange="window.location = this.options[this.selectedIndex].value; return true;">
