...
// $SQL = "Select * From cotizaciones Where tpEnsayo != 2 and  Estado = 'T' and RAM > 0 and nSolicitud = 0 Order By Facturacion, Archivo, informeUp, fechaTermino Desc";
// $SQL = "SELECT * FROM cotizaciones where Estado = 'T' and (nSolicitud = 0 or nSolicitud IS NULL) and RAM > 0 and (Archivo != 'on' or Archivo IS NULL) and (informeUp != 'on' or informeUp IS NULL) Order By Facturacion, Archivo, informeUp, fechaTermino Desc";
$SQL = "SELECT * FROM cotizaciones where Estado = 'T' and (nSolicitud = 0 or nSolicitud IS NULL) and RAM > 0 and (Archivo != 'on' or Archivo IS NULL)  Order By Facturacion, Archivo, informeUp, fechaTermino Desc";
$bd=$link->query($SQL);
while($rs = mysqli_fetch_array($bd)){
		...
		$infoNumero = 0;
		$infoSubidos = 0;
        if($rs["infoNumero"] == NULL ){ $color = 'blanca'; }
        if($rs["nOC"]){ $ordenCompra = 'Si'; }

        if($rs["infoSubidos"] == NULL || $rs["infoSubidos"] < $rs["infoNumero"]){
        	$color = 'blanco';
        }
