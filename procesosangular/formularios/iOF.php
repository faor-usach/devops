		...
		if($this->PageNo() > 1){
			...
			if($rowCo=mysqli_fetch_array($bdCo)){
				...
				if($rowCo['IvaUF'] == 0) { 
					...
					$totalOfertaUF .= utf8_decode(' más IVA'); 
				}
			}
			...
			$ln = 5;
			$this->SetXY(70,$ln);
			$this->Cell(50,4,utf8_decode('Oferta Técnico Económica'),0,0,'C');
			...
			$this->SetXY(137,$ln);
			$this->Cell(10,4,utf8_decode('Revisión : 0'),0,0);
			...
			$this->SetFont('Arial','',10);
			$this->SetXY(137,$ln);
			$this->Cell(10,4,utf8_decode('Página : ').$this->PageNo().' de {nb}',0,0);
...
$pdf->AddPage();
	...
	if($rowOF=mysqli_fetch_array($bdOF)){
		...
		if($rowCli=mysqli_fetch_array($bdCli)){
			...
			$pdf->SetXY(150,$ln);
			$pdf->Cell(100,5,utf8_decode('N° Páginas ').'{nb}',0,0);
		...
		if($rowCli=mysqli_fetch_array($bdCli)){
			...
			$ln += 5;
			$pdf->SetXY(15,$ln);
			$pdf->Cell(30,5,utf8_decode('Razón Social'),0,0,'L');
			$pdf->SetXY(40,$ln);
...
*/											
											...
											if($rowIe['Generico'] != 'on'){
												...
												if($rowEA=mysqli_fetch_array($bdEA)){
													do{
														...
														$pdf->SetFont('Arial','',10);
														$pdf->SetX(35);
														$pdf->MultiCell(150,5,'° '.utf8_decode($rowEA['Descripcion']),0,'J');
	...
	$pdf->SetFont('Arial','B',10);
	$pdf->SetX(15);
	$pdf->Cell(30,5,$nItem.utf8_decode('.1.- Envío de Muestras y Horario:'),0,0,'L');
	...
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(20);
	$pdf->MultiCell(160,5,utf8_decode(' Av. El belloto N° 3735, Estación Central, Santiago.'),0);
	...
	$pdf->SetX(20);
	$pdf->MultiCell(160,5,utf8_decode('- Departamento de Ingeniería Metalurgia, Sector Fundición, Laboratorio de Ensayos e Investigación de Materiales SIMET-USACH.'),0);
	...
	$pdf->SetX(20);
	$pdf->MultiCell(160,5,utf8_decode('- Horario de Atención: Lunes a Jueves 9:00 a 13:00 hrs // 14:00 a 18:00 hrs Viernes 9:00 a 13:00 hrs // 14:00 a 16:30 hrs., previa coordinación para piezas grandes.'),0);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(20);
	$pdf->MultiCell(160,5,utf8_decode('- Tipo de moneda, en pesos, según valor de la UF correspondiente al día de emisión de la Orden de Compra o Factura.'),0);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(20);
	$pdf->MultiCell(160,5,utf8_decode('- La forma de pago será contra factura:'),0);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(25);
	$pdf->MultiCell(160,5,utf8_decode("- Pago mediante depósito o transferencia a nombre de SDT USACH, Banco BCI cuenta corriente 10358391 Rut: 78172420-3. Enviar confirmación a simet@usach.cl."),0);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(25);
	$pdf->MultiCell(160,5,utf8_decode("- Clientes nuevos, sólo pago anticipado."),0);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(20);
	$pdf->MultiCell(160,5,utf8_decode('- Después de 10 días de corridos de la emisión de este informe se entenderá como aceptado en su versión final, cualquier modificación posterior tendrá un recargo adicional de 1UF + IVA.'),0);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(20);
	$pdf->MultiCell(160,5,utf8_decode('- Se solicita indicar claramente la identificación de la muestra al momento de la recepción, para no rehacer informes. Cada informe rehecho por razones ajenas a SIMET-USACH tiene un costo de 1,00 UF + IVA.'),0);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(20);
	$pdf->MultiCell(160,5,utf8_decode('- Visitas a terreno en Santiago, explicativas de informes de análisis de falla o de retiro de muestras en terreno, tienen un costo adicional de 6,0 UF + IVA, visitas fuera de la región metropolitana consultar.'),0);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(20);
	$pdf->MultiCell(160,5,utf8_decode('- En caso de realizar análisis de falla, el laboratorio se reserva el derecho de modificar el tipo y/o cantidad de ensayos.'),0);
	...
	$pdf->SetXY(60, $ln);
	$pdf->Cell(100,5,utf8_decode(': SOCIEDAD DE DESARROLLO TÓCNOLOGICO USACH LTDA.'),0,0,'L');
	...
	$pdf->SetXY(60, $ln);
	$pdf->Cell(100,5,utf8_decode(': Educación, Capacitación y Publicidad'),0,0,'L');
	...
	$pdf->SetXY(60, $ln);
	$pdf->Cell(100,5,utf8_decode(": Emma Barceló Araos"),0,0,'L');
