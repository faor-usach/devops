			...
			if($rowc=mysqli_fetch_array($bdc)){
				...
				for($i=1; $i <= $_POST['nInforme']; $i++){
					...
					if($rowiam=mysqli_fetch_array($bdiam)){
						...
						$bdinformes=$link->query("SELECT * FROM informes Where CodInforme = '$CodInfNew'");
						if($rsinformes=mysqli_fetch_array($bdinformes)){
							echo $iam.' -> '.$_POST['nInforme'].' CodInf. ->'.$CodInforme.'<br>';
							...
							// echo $iam.' -> '.$_POST['nInforme'].'CREANDO... CodInf. ->'.$CodInforme.'<br>';
		    ...
		    <tbody>
				<?php
					...
					//}else{
						...
						$bdCot=$link->query("SELECT * FROM cotizaciones Where RAM > 0 and Estado = 'P' and Archivo IS NULL and Eliminado IS NULL");
					//}
